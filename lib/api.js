...
export function createAgentWebSocket() {
    ...
    if (typeof e.data === 'string') {
      try {
        ...
        if (msg.type === 'interrupted') {
          // Backend cancelled the old turn because a new message arrived.
          // Decrement skipResponses so the upcoming new response is not discarded.
          if (skipResponses > 0) skipResponses--;
        ...
        if (msg.type === 'done') {
          ...
          pending.textData = {
            ...
            scroll_to: msg.scroll_to ?? null,
          };
          // Fire immediately — before 'done' and before audio playback begins
          pending.onTextReceived?.(pending.textData);
  ...
  return {
    ...
    send: (message, conversationHistory, extractedData, onAudioChunk, websiteContext, onTextReceived) =>
      new Promise((resolve, reject) => {
        ...
        pending = { resolve, reject, chunks: [], onAudioChunk, onTextReceived, chunkQueue: Promise.resolve() };
        try {
          const payload = {
            message,
            conversation_history: conversationHistory,
